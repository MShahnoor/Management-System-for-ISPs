...
export default function PaymentsData() {
  ...
  const getData = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3001/api/payment/getPayments");
  ...
  React.useEffect(() => {
    setRows(
      payments.map((obj) => ({ ...obj, actions: <ActionIcons obj={obj} /> }))
    );
  }, [payments]);
  ...
  if (isLoading) {
    ...
    return (
      ...
      >
        <h1 style={{ padding: 20 }}>No Payments Found</h1>
